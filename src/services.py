...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("services.log"), logging.StreamHandler()],
)
...
def get_person_transfers(transactions):
    ...
    try:
        ...
        for tx in transactions:
            if (
                tx.get("category") == "Переводы"
                and isinstance(tx.get("description"), str)
                and pattern.match(tx["description"].strip())
            ):
